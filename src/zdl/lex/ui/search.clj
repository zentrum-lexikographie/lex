...
(defn suggest
  [q]
  (t/with-ctx+ {::suggest-query q}
    (try (client/http-suggest q) (catch Throwable t (t/error! t) []))))
...
(proxy [AbstractListIntelliHints] [input]
  ...
  (updateHints [ctx]
    (let [q           (str ctx)
          suggestions [] #_(and (suggest? q) (suggest q))
  ...
  (let [filter-action (ui/action
    ...
    (ui/horizontal-panel
     :items [(Box/createRigidArea (to-dimension [5 :by 0]))
             (ui/label :text (.format time-formatter timestamp)
                       :font (util/derived-font :style :plain))

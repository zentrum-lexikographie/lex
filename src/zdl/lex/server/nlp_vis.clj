...
(defn dep-tree-nodes
  [{:keys [tokens deps]}]
  (let [dep-tree-levels    (into {} (dep-tree-levels deps (:n (first #_deps/root tokens))))
...
(defn document
  ...
  (let [sentence (identity #_deps/assoc-deps sentence)]
    [:dali/page
     [:defs
...
(defn show!
  [sentence]
  (let [temp-svg (fs/create-temp-file {:prefix "zdl-nlp-vis-" :suffix ".svg"})]

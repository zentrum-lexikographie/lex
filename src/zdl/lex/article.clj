...
(defn parse-article
  ...
  (let [last-modified (or
    ...
    (cond-> {:last-modified last-modified
             :weight        (try
                              (or (some-> last-modified ts/parse
                                          ts/days-since-epoch) 0)
                              (catch Throwable t
                                (t/error! {:error t :level :debug}) 0))}
      type             (assoc :type type)
...
(def xml-template
  (read-xml (io/resource "template.xml")))
  ...
  (->str
   (postwalk
    (fn [node]
      (if (map? node)
        (condp = (node :tag)
          :Artikel    (let [ts (ts/format (java.time.LocalDate/now))]

...
def prepare():
    logger.info("Preparing UD-HDT dataset")
    url = (
    ...
    def read_docs(zf, name):
        logger.info(f"Converting UD-HDT file '{name}'")
    ...
    def extract(zf):
        ...
        (Path(__file__).parent / "hdt.train.spacy").write_bytes(data)

        for split in ("dev", "test"):
            docs = read_docs(zf, f"{path_prefix}{split}.conllu")
            data = DocBin(docs=docs, store_user_data=True).to_bytes()
            (Path(__file__).parent / f"hdt.{split}.spacy").write_bytes(data)

            ...
            for doc in jsonstream.loads(gzip.decompress(data)):
                ...
                for token in doc["tokens"]["array"]:
                    ...
                    if len(tag) == 2:
                        iob, ent = tag
                        ent = tags[ent]
                        tag = f"{iob}-{ent}" if ent != "O" else ent
                    else:
                        tag = tag[0]
...
def his_german_ner(write):
    ...
    for split in ("train", "dev", "test"):
        ...
        with open_text(urlopen(url)) as f:
            sentence = []
            for line in f:
                if line.startswith(("#", "TOKEN")) or "DOCSTART" in line:
                    continue
                elif not line.strip():
                    if sentence:
        ...
        for split in ("train", "dev", "test"):
            ...
            with open_text(zf.open(name)) as f:
                ...
                for line in f:
                    ...
                    if "PySBDSegment" in misc and sentence:
...
def open_ner_split(split):
    return bz2.open(
        Path(__file__).parent / f"ner-d.{split}.tsv.bz2", "wt", encoding="utf-8"
    )
...
def prepare():
    for split in ("train", "dev", "test"):
        logger.info(f"Preparing NER-d/{split}")
        file_name = f"ner-d.{split}.tsv.bz2"
        with bz2.open(Path(__file__).parent / file_name, "rt", encoding="utf-8") as f:
            docs = conll_ner_to_docs(f.read(), n_sents=32, no_print=True)
            data = DocBin(docs=docs, store_user_data=True).to_bytes()
            (Path(__file__).parent / f"ner-d.{split}.spacy").write_bytes(data)

...
(def current-timestamp
  (let [odt (OffsetDateTime/now (java.time.Clock/systemUTC))]
...
(defn timestamp
  [& _]
  (t/log! :info (format "Timestamp: %s" current-timestamp))
  (spit (fs/file "src" "timestamp.txt") current-timestamp))

(defn schema
  [& _]
  (t/log! :info "Transpiling DWDSwb schema")
...
(defn client
  ...
  (timestamp)
  (schema)
  (t/log! :info "Compiling Oxygen XML Editor plugin")
